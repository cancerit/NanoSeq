...
parserR = parser.add_argument_group('required arguments') #need this to get opt/req args correctly
parserO.add_argument('--out',   action='store', default='.', help='path of the output files and scratch directory (.)')
parserO.add_argument('-j','--index', type=int, action='store', help='index of the LSF job array. One based')
parserO.add_argument('-k','--max_index', type=int, action='store', help='maximum index of the LSF job array')
parserO.add_argument('-t','--threads', type=int, action='store', default= 1, help='number of threads (1)')
parserR.add_argument('-R','--ref', action='store', required=True, help="referene sequence")
parserR.add_argument('-A','--normal', action='store', required=True, help="normal BAM / CRAM")
parserR.add_argument('-B','--tumour', action='store', required=True, help="tumour (duplex) BAM / CRAM")
parserO.add_argument('-v','--version', action='version', version=version)
parser._action_groups.append(parserO)#need this to get opt/req args correctly
...
parser_covO.add_argument('--exclude', action='store', default='MT,GL%%,NC_%,hs37d5', help='List of contigs to exclude. Comma separated, %% acts as a wild card. (MT,GL%%,NC_%%,hs37d5)')
parser_covO.add_argument('--include', action='store', help='Only include these contigs. Comma separated, %% acts as a wild card.')
parser_covO.add_argument('--larger', type=int, action='store', default=0, help='Only include contigs larger than this size. (0)')
parser_covO.add_argument('-w','--win', type=int, action='store', default=100, help='bin size for coverage distribution (100)')
parser_covO.add_argument('-Q', type=int, action='store', default=0, help="minimum mapQ to include a tumour read (0)")
...
parser_partR.add_argument('-n','--jobs', type=int, action='store', required=True, help='partition dsa,var,indel to this many tasks')
parser_partO.add_argument('--excludeBED', action='store', help='BED (gz) file with regions to exclude from analysis.')
parser_partO.add_argument('--excludeCov', type=int, action='store', help='Exclude regions with coverage values higher than this')
...
parser_dsaR.add_argument('-C','--snp', action='store', required=True, help="SNP BED (gz) file")
parser_dsaR.add_argument('-D','--mask', action='store', required=True, help="mask BED (gz) file")
parser_dsaO.add_argument('-d', type=int, action='store', default=2, help="minimum duplex depth (2)")
parser_dsaO.add_argument('-q', type=int, action='store', default=30, help="minimum base quality for normal (30)")
parser_dsaO.add_argument('--no_test', action='store_true', help="skip BAM format tests, use with caution" )
...
parser_varO.add_argument('-a', type=int, action='store', default=50, help="minimum AS-XS (50)")
parser_varO.add_argument('-b', type=int, action='store', default=5, help="minimum matched normal reads per strand (5)")
parser_varO.add_argument('-c', type=int, action='store', default=0, help="maximum number of clips (0)")
parser_varO.add_argument('-d', type=int, action='store', default=2, help="minimum duplex depth (2)")
parser_varO.add_argument('-f', type=float, action='store', default=0.9, help="minimum fraction of reads for consensus (0.9)")
parser_varO.add_argument('-i', type=float, action='store', default=1.0, help="maximum fraction of reads with an indel (1.0)")
parser_varO.add_argument('-m', type=int, action='store', default=8, help="minimum cycle number (8)")
parser_varO.add_argument('-n', type=int, action='store', default=3, help="maximun number of mismatches (3)")
parser_varO.add_argument('-p', type=int, action='store', default=0, help="minimum fraction of reads that are proper-pairs (0)")
parser_varO.add_argument('-q', type=int, action='store', default=60, help="minimum consensus base quality (60)")
parser_varO.add_argument('-r', type=int, action='store', default=144, help="read length (after 5' trimming) (144)")
parser_varO.add_argument('-v', type=float, action='store', default=0.01, help="maximum bulk VAF (0.01)")
parser_varO.add_argument('-x', type=int, action='store', default=8, help="maximum cycle number (8)")
parser_varO.add_argument('-z', type=int, action='store', default=12, help="minimum total number of normal reads (12)")
...
parser_indelO.add_argument('-s','--sample', action='store', default='sample_1', help="sample name in output vcf (sample_1)")
parser_indelO.add_argument('--rb', type=int, action='store', default=2, help="minimum reads in a bundle. (2)")
parser_indelO.add_argument('--t3', type=int, action='store', default=135, help="excess bases above this value are trimmed from 3' (135)")
parser_indelO.add_argument('--t5', type=int, action='store', default=10, help="bases to trim from 5' reads (10)")
parser_indelO.add_argument('--mc', type=int, action='store', default=16, help="minimum bulk coverage (16)")
