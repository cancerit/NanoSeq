...
foreach my $rb_id (keys %indels) {
  ...
  while(<IN_TMP>) {
    if(/^#/) {
      if($get_header == 1) {
        ...
        if ($once and /^##FORMAT/) { # should be ##FORMAT, edited for syntax highlighting
  ...
  close(IN_TMP);
  $vcf_content .= $info[$best_count] if (  @info > 0 && $best_count != -1 ); # fa8 No longer printing MASKED or BULK_SEEN
